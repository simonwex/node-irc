    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "330": // rpl_whoisaccount?
                ...
                break;
            case "err_nosuchnick":
                // Check if we're waiting on a whois for the non-existent nick
                if (self._whoisData[message.args[1]])
                    //TODO: Check and see what else might be waiting on a 401
                    self._addWhoisData(message.args[1], 'error', message.args[2]);
                break;
            case "rpl_endofwhois":
...
Client.prototype.whois = function(nick, callback) { // {{{
    if ( typeof callback === 'function' ) {
        ...
        // Let the system know we're waiting for this:
        this._whoisData[nick] = {nick: nick};
